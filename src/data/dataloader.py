...
class STDataloader:
    ...
    def __call__(self, *args, **kwargs):
        ...
        for path in self.content_datapath:
            content_datapath_list = os.listdir(path)
            for url in content_datapath_list:
                if os.path.isfile(os.path.join(path, url)):
                    extension = url.split(".")[-1]
                    assert extension in ["jpg"] or extension in ["png"], "non-image file found in dir."
                    content_image_urls.append(os.path.join(path, url))
                else:
        ...
        for path in self.style_datapath:
            style_datapath_list = os.listdir(path)
            for url in style_datapath_list:
                if os.path.isfile(os.path.join(path, url)):
                    extension = url.split(".")[-1]
                    assert extension in ["jpg"] or extension in ["png"], "non-image file found in dir."
                    style_image_urls.append(os.path.join(path, url))
                else:
                    warnings.warn("Non file found in data dir")
                    continue

        dataset = STDataset(content_image_urls[:self.max_content_train_samples],
